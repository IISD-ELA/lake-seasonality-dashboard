...
#         try:
#             j = r.json()
#             st.write("Raw JSON (j):", j)
#         except Exception as e:
#             st.error("Failed to parse JSON")
#             st.write("Raw text:", r.text)
#             raise

#         # Handle RIE / Lambda wrapper
#         payload = j.get("body") if isinstance(j, dict) and "body" in j else j
#         st.write("Payload (before parsing):", payload)

#         if isinstance(payload, str):
#             try:
#                 payload = json.loads(payload)
#                 st.write("Payload (after json.loads):", payload)
#             except JSONDecodeError:
#                 st.error("Failed to decode payload string")
#                 payload = {}

#         # Extract data
#         data = payload.get("data", []) if isinstance(payload, dict) else []
#         st.write("Final data field:", data)

#         if isinstance(data, list) and len(data) > 0:
#             st.write("Sample row:", data[0])
#         else:
#             st.warning("Data is empty or not a list")

#     # Final DataFrame
#     df = pd.DataFrame.from_records(data if isinstance(data, list) else [])

#     if "start_date" not in df.columns:
#         st.error("ERROR 'start_date' column missing in DataFrame")
